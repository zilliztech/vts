...
public class MilvusSinkConfig extends MilvusCommonConfig {
     ...
     *
     * IMPORTANT: When field_schema is supplied, ONLY the fields defined in field_schema will be used.
     * The schema is built entirely from the configuration - source schema is not referenced.
     *
     * If field_schema is NOT supplied (empty), the full source schema will be used by default.
     *
     ...
     *   OR
     * - source_field_name: For dynamic field extraction, the path in source dynamic field (String)
     *   - If both are provided, field_name is used as target name
     *   - If only source_field_name is provided, it's used as both source path and target name
     * - data_type: Milvus data type code (Integer, REQUIRED for all fields)
     *   Common types: Int64=5, VarChar=21, FloatVector=101, etc.
     *
     * Optional type-specific fields:
     * - element_type: Element type for Array fields (Integer, required for Array type)
     * - max_capacity: Max capacity for Array fields (Integer, optional, default: 4096)
     * - max_length: Max length for VarChar/String fields (Integer, optional, default: 65535)
     * - dimension: Dimension for vector fields (Integer, required for vector types)
     *
     * Optional field properties:
     * - is_nullable: Whether the field is nullable (Boolean, optional, default: true)
     * - default_value: Default value for the field (Object, optional)
     * - is_primary_key: Mark field as primary key (Boolean, optional, default: false)
     * - auto_id: Enable auto ID for primary key field (Boolean, optional)
     * - is_partition_key: Mark field as partition key (Boolean, optional, default: false)
     * - enable_analyzer: Enable analyzer for full-text search (Boolean, optional, default: false)
     * - analyzer_params: Analyzer parameters (Map<String, Object>, optional)
     * - enable_match: Enable match for text matching (Boolean, optional, default: false)
     *
     ...
     *   {field_name = "category", data_type = 21, max_length = 100, is_partition_key = true}
     * ]
     *
     * Example - Extract from dynamic fields:
     * field_schema = [
     *   {source_field_name = "metadata.title", field_name = "title", data_type = 21, max_length = 200}
     *   {source_field_name = "metadata.tags", field_name = "tags", data_type = 22, element_type = 21}
     * ]
     */
    public static final Option<List<Object>> FIELD_SCHEMA =
            Options.key("field_schema")
                    .listType(Object.class)
                    .defaultValue(new ArrayList<>())
                    .withDescription("Field schema configuration. When supplied, ONLY fields defined here will be used (built entirely from config). If empty, uses full source schema.");
    /**
